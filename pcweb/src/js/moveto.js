...
(function($) {
	$.fn.extend({
		moveTo: function(target, iType, fnEnd, iDate) {
			var i = 0;
			for(i = 0; i < this.length; i++) { move(this[i], target, iType, fnEnd, iDate); }

			function move(obj, target, iType, fnEnd, iDate) {
				if(obj.timer) {
				...
				switch(iType) {
					case 0:
						obj.timer = setInterval(function() { doMoveBuffer(obj, target, fnEnd); }, time);
						break;
					case 1:
						obj.timer = setInterval(function() { domoveFlexible(obj, target, fnEnd); }, time);
						break;
				}
